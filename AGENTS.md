...
- Local patch branches are pushed to the `fork` remote (public fork `KAITO-XI/deepseek-harness-desktop`; its public visibility is intentional and accepted) and optionally to `backup` (private mirror `KAITO-XI/dsh-desktop-local`). Never push `origin` (upstream) or `mirror`.
